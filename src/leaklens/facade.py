...
class CrawlerFacade:
    ...
    def start(self):
        ...
        try:
            ...
            if self.detail_output:
                ...
                # API Endpoints
                self.formatter.output_api_endpoints(self.crawler.api_endpoints, True)
                # Authentication Detection Results
                if hasattr(self.crawler, 'auth_results') and self.crawler.auth_results:
                    self.formatter.output_auth_results(self.crawler.auth_results, True)
                # IDOR Detection Results
                if hasattr(self.crawler, 'idor_results') and self.crawler.idor_results:
                    self.formatter.output_idor_results(self.crawler.idor_results, True)
                # JWT Detection Results
                if hasattr(self.crawler, 'jwt_results') and self.crawler.jwt_results:
                    self.formatter.output_jwt_results(self.crawler.jwt_results, True)
            else:
                # tidy output
                # URLs per domain
                domains = set()
    ...
    def create_crawler(self) -> Crawler:
        ...
        # API, Auth, IDOR, and JWT detection
        api_detection: typing.Optional[bool] = self.custom_settings.get("api_detection", True)
        auth_detection: typing.Optional[bool] = self.custom_settings.get("auth_detection", False)
        idor_detection: typing.Optional[bool] = self.custom_settings.get("idor_detection", False)
        jwt_detection: typing.Optional[bool] = self.custom_settings.get("jwt_detection", False)
        auth_token: typing.Optional[str] = self.custom_settings.get("auth_token", None)
        ...
        crawler = Crawler(
            ...
            idor_detection=idor_detection,
            jwt_detection=jwt_detection,
