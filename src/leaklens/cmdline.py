...
from leaklens import __version__
from leaklens.config import settings
from leaklens.exception import FacadeException
...
rules:
  ...
  - name: Email
    ...
    loaded: true
  - name: Public IP
    regex: '[^0-9]((?!(127\.0\.0\.1|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.((1[6-9])|(2\d)|(3[01]))\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}))(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))'
    loaded: true
  - name: Cloud Key
